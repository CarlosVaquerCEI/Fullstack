...
const connect = async () => {
    ...
    const options = {
        collection: 'usuarios',
        strict: true, // solo guarda los elementos que estén en nuestro esquema
        collation: {
            locale: "es",
...
app.post("/usuarios2", async (req, res, next) => {
    const nuevoUsuario = new Alumno ({
        ...
        edad: 18,
        isAdmin: true,
        hobbies: ["fútbol", "básquet", "programación"],
...
app.put("/usuarios/:id", async (req, res, next) => {
    ...
    try {
        ...
        const alumnoEditado = await Alumno.findByIdAndUpdate(id, { name, username }, {new: true}) // new: true nos va adevolver el usuario actualizado
...
app.delete("/usuarios/:id", async (req, res, next) => {
    try {
        const {id} = req.params
        const alumnnoEliminado = await Alumno.findByIdAndDelete(id)

        res.json(alumnnoEliminado)
    } catch (error) {
        res.json(error)
    }
...
app.get("/operadores", async (req, res, next) => {
    ...
    // Usuarios que jueguen al fútbol o al volley
    const jugadoresDeFutbol = await Alumno.find({hobbies: {$in: ["futbol", "volley"]}})

    // Usuerios que tengan el campo hobbies
    const userConHobbies = await Alumno.find({hobbies: {$exists: true}})

    res.json(userConHobbies)
})
